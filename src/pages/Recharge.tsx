...
const Recharge = () => {
  ...
  const handleConfirm = () => {
    ...
    if (!parsedAmount || parsedAmount < minAmount) {
      showError("Erreur", `Le montant minimum est de ${minAmount.toLocaleString()} CDF`);
      return;
    }
    if (parsedAmount > maxAmount) {
      showError("Erreur", `Le montant maximum est de ${maxAmount.toLocaleString()} CDF`);
      return;
    }
    if (!selectedMethod) {
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="px-4 pt-5 space-y-5">
        ...
        <div className="bg-card rounded-2xl border border-border/30 p-4">
          <label className="text-xs text-muted-foreground mb-2 block">Montant (CDF)</label>
          <div className="flex items-center bg-secondary/50 rounded-xl px-4 py-3">
            ...
            />
            <span className="text-muted-foreground font-semibold text-sm ml-3 whitespace-nowrap">CDF</span>
          </div>
