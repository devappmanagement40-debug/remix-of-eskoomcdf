...
const MesProduits = () => {
  ...
  const handleCollect = async (up: UserProduct) => {
    ...
    try {
      ...
      showSuccess("Gains collectés", `+${Number(data.amount).toLocaleString("fr-FR")} XAF crédités sur votre compte`);
      load();
    } catch (err) {
          ...
          <div className="bg-card rounded-xl border border-secondary p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-secondary/50 rounded-lg p-3">
                ...
                <p className="text-sm font-bold text-foreground">{Number(product?.price).toLocaleString("fr-FR")} XAF</p>
              </div>
              {gainType === "daily" && (
                <div className="bg-secondary/50 rounded-lg p-3">
                  <p className="text-[10px] text-muted-foreground">Revenu quotidien</p>
                  <p className={`text-sm font-bold ${seriesTextColors[color] || "text-success"}`}>{dailyRevenue.toLocaleString("fr-FR")} XAF</p>
                </div>
              )}
              <div className="bg-secondary/50 rounded-lg p-3">
                <p className="text-[10px] text-muted-foreground">{gainType === "blocked" ? "Gain prévu" : "Revenu total"}</p>
                <p className={`text-sm font-bold ${seriesTextColors[color] || "text-primary"}`}>{totalRevenue.toLocaleString("fr-FR")} XAF</p>
              </div>
              <div className="bg-secondary/50 rounded-lg p-3">
              ...
              <div className="bg-secondary/50 rounded-lg p-3">
                ...
                <p className="text-sm font-bold text-foreground">{Number(earnedSoFar).toLocaleString("fr-FR")} XAF</p>
              </div>
              {gainType === "daily" && (
                <div className="bg-secondary/50 rounded-lg p-3">
                        ...
                        <>
                          ...
                          {earnedSoFar > 0 && (
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-sm text-foreground">{Number(earnedSoFar).toLocaleString("fr-FR")} XAF</span>
                            </div>
                          )}
                        </>
                      ) : (
