...
const Products = () => {
  ...
  const checkSeriesAccess = (s: Series): string[] => {
    ...
    if ((s.min_personal_investment || 0) > 0 && userAccess.personalInvestment < (s.min_personal_investment || 0)) {
      missing.push(`Investissement personnel de ${Number(s.min_personal_investment).toLocaleString("fr-FR")} CDF requis`);
    }
    if ((s.min_team_investment || 0) > 0 && userAccess.teamInvestment < (s.min_team_investment || 0)) {
      missing.push(`Investissement équipe de ${Number(s.min_team_investment).toLocaleString("fr-FR")} CDF requis`);
    }
  ...
  const handlePurchase = async (product: Product) => {
    ...
    try {
      ...
      if (totalBalance < price) {
        showError("Solde insuffisant", `Votre solde (${totalBalance.toLocaleString("fr-FR")} CDF) est insuffisant pour acheter ce produit (${price.toLocaleString("fr-FR")} CDF). Veuillez recharger votre compte.`);
        return;
      }
                    ...
                    <div className="flex flex-col gap-1 flex-1 min-w-0">
                      ...
                      <div className="grid grid-cols-2 gap-x-2 gap-y-0.5 mt-1">
                        <div>
                          ...
                          <p className="text-xs font-bold text-primary">{Number(product.total_revenue).toLocaleString("fr-FR")} <span className="text-[9px] font-normal text-muted-foreground">CDF</span></p>
                        </div>
                        <div>
                          <p className="text-[9px] text-muted-foreground">Revenu Quotidien</p>
                          <p className="text-xs font-bold text-primary">{Number(product.daily_revenue).toLocaleString("fr-FR")} <span className="text-[9px] font-normal text-muted-foreground">CDF</span></p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <p className="text-xs font-bold text-primary">{Number(product.price).toLocaleString("fr-FR")} <span className="text-[9px] font-normal text-muted-foreground">CDF</span></p>
                        </div>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-3 pt-2">
                ...
                <div className="text-center space-y-1">
                  ...
                  <p className="text-lg font-bold text-primary">{Number(confirmProduct?.price || 0).toLocaleString("fr-FR")} CDF</p>
                </div>
