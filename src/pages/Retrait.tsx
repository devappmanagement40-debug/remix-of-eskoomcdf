...
const Retrait = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (numAmount < minAmount) { showError("Erreur", `Montant minimum : ${minAmount} USDT`); return; }
    if (numAmount > maxAmount) { showError("Erreur", `Montant maximum : ${maxAmount.toLocaleString()} USDT`); return; }
  ...
  return (
    <div className="min-h-screen bg-background pb-10">
      ...
      <div className="px-4 pt-6 space-y-4">
        ...
        {/* Amount */}
        <div className="bg-card rounded-2xl border border-border/30 p-4">
          <label className="text-xs text-muted-foreground mb-2 block">Montant du retrait (USDT)</label>
          <input
            type="number"
            value={amount}
