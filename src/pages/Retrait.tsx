...
const Retrait = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (numAmount < minAmount) { showError("Erreur", `Montant minimum : ${minAmount} CDF`); return; }
    if (numAmount > maxAmount) { showError("Erreur", `Montant maximum : ${maxAmount.toLocaleString()} CDF`); return; }
    ...
    <div className="min-h-screen bg-background pb-10">
      ...
      <div className="px-4 pt-6 space-y-4">
        ...
        <div className="bg-card rounded-2xl border border-border/30 p-5 text-center">
          ...
          <p className="text-3xl font-bold text-foreground">{withdrawableBalance.toLocaleString("fr-FR")} <span className="text-sm font-normal text-muted-foreground">CDF</span></p>
        ...
        {/* Amount */}
        <div className="bg-card rounded-2xl border border-border/30 p-4">
          <label className="text-xs text-muted-foreground mb-2 block">Montant du retrait (CDF)</label>
          <input
            type="number"
            value={amount}
          ...
          {numAmount > 0 && (
            <div className="mt-3 space-y-1.5 pt-3 border-t border-border/20">
              <div className="flex justify-between text-xs">
                ...
                <span className="text-foreground font-semibold">{numAmount.toLocaleString("fr-FR")} CDF</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-muted-foreground">Frais ({feePercent}%)</span>
                <span className="text-destructive font-semibold">- {feeAmount.toLocaleString("fr-FR")} CDF</span>
              </div>
              <div className="flex justify-between text-sm pt-1">
                <span className="text-foreground font-bold">Vous recevrez</span>
                <span className="text-success font-bold">{netAmount.toLocaleString("fr-FR")} CDF</span>
              </div>
            </div>
          )}
        </div>
